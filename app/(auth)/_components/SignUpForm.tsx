...
import { KeyIcon, MailIcon } from "@/app/_components/ui/Icons";
import { showToast } from "@/app/_hooks/useToast";

import FormInput from "../../_components/Forminput";
import Label from "../../_components/Label";

type PasswordType = Omit<Input, "email">;
...
const SignUpForm: React.FC = () => {
  ...
  function handleValidations(e: FormEvent) {
    ...
    keys.forEach((key) => {
      const input = inputs[key as keyof Input].trim();
      ...
      if (!input) {
        ...
        e.preventDefault();
        keyError[key as keyof PasswordType] = "Must be upto 8 characters";
      } else {
        setError({});
      }
    });
  ...
  async function handleSubmit(formData: FormData) {
    const sendData = await axios.post("/api/auth/signup", formData);
      ...
      <div className=" flex flex-col gap-5 xs:w-[calc(100%-20px)] sm:!w-[480px] ">
        ...
        <div className="bg-white rounded-md grid gap-10 p-5">
          <div className="flex flex-col gap-3">
            <h1 className="text-2xl text-gray font-bold">Create Account</h1>
          ...
          >
            ...
            <div className="flex flex-col gap-5">
              ...
              <p className="text-darkGray text-sm font-semibold">
